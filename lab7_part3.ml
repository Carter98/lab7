...
module IntListStack =
  struct
    ...
    (* top s -- Returns the value of the topmost element on stack s,
       ...
       returned. *)
    let top (s : stack) : int =      match s with      | [] -> raise EmptyStack      | h :: _ -> h

    (* pop s -- Returns a stack with the topmost element from s
       removed, raising the EmptyStack exception if there is no
       element to be removed. *)
    let pop (s : stack) : stack =      match s with      | [] -> raise EmptyStack      | _ :: t -> t  
  end ;;
...
let small_stack () : IntListStack.stack =  let open IntListStack in  empty  |> push 5  |> push 1 ;;
...
module type INT_STACK =  sig    exception EmptyStack    type stack    val empty : stack    val push : int -> stack -> stack    val top : stack -> int    val pop : stack -> stack  end ;;
...
let safe_stack () : SafeIntListStack.stack =  let open SafeIntListStack in  empty  |> push 5  |> push 1 ;;
